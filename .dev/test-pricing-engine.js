...
const row = (line, pid, unit, factor, min, max, cash, credit, label) => ({ record_id: itemId++, price_list_id: 1, line_id: line, product_id: pid, unit_code: unit, unit_factor: factor,
  min_qty: min, max_qty: max === null ? '' : max, cash_price_incl_vat: cash, credit_price_incl_vat: credit, van_only: unit === 'PACK' ? 'TRUE' : 'FALSE', tier_label: label || '' });
const items = [];
// line 1: เอ็กซ์ตรีม แซนดัลวูด(101) + ลาเวนเดอร์(102) ใช้ตารางขั้นร่วมกัน
[101, 102].forEach(pid => {
  [[1, 1, 1210, 1225], [2, 9, 1200, 1215], [10, 29, 1190, 1205], [30, 49, 1180, 1195], [50, null, 1170, 1185]].forEach(t => items.push(row(1, pid, 'CASE', 60, t[0], t[1], t[2], t[3])));
  items.push(row(1, pid, 'PACK', 5, 1, null, 101, '', 'ขายเฉพาะหน่วยรถ'));
});
// line 2: กาวแถมกาวสองหน้า (201) 1x20x5x5
[[1, 1, 1240, 1255], [2, 4, 1220, 1235], [5, null, 1200, 1215]].forEach(t => items.push(row(2, 201, 'CASE', 500, t[0], t[1], t[2], t[3])));
...
let r = price([{ productId: 101, unitCode: 'CASE', qty: 1 }], cash);
eq('1 หีบ เงินสด = 1,210', [r.success, r.lines[0].unitPrice, r.total], [true, 1210, 1210]);

r = price([{ productId: 101, unitCode: 'CASE', qty: 1 }], credit);
eq('1 หีบ เครดิต = 1,225 (เงินสด +15)', r.lines[0].unitPrice, 1225);

r = price([{ productId: 101, unitCode: 'CASE', qty: 1 }, { productId: 102, unitCode: 'CASE', qty: 1 }], cash);
...
r = price([{ productId: 101, unitCode: 'CASE', qty: 30 }], cash);
eq('30 หีบ (ภาค กทม.) = ขั้น 30-49 = 1,180', r.lines[0].unitPrice, 1180);

r = price([{ productId: 101, unitCode: 'CASE', qty: 50 }], credit);
eq('50 หีบ เครดิต = ขั้น 50+ = 1,185', r.lines[0].unitPrice, 1185);

r = price([{ productId: 101, unitCode: 'PACK', qty: 3 }], { isCredit: false, isVan: true });
eq('แพ็ค Cash Van เงินสด 3 แพ็ค = 303', [r.success, r.lines[0].unitPrice, r.total], [true, 101, 303]);

r = price([{ productId: 101, unitCode: 'PACK', qty: 1 }], { isCredit: false, isVan: false });
eq('แพ็คนอก Cash Van → ปฏิเสธ', [r.success, r.code], [false, 'PACK_VAN_ONLY']);

r = price([{ productId: 101, unitCode: 'PACK', qty: 1 }], { isCredit: true, isVan: true });
eq('แพ็คแบบเครดิต → ปฏิเสธ', [r.success, r.code], [false, 'PACK_CASH_ONLY']);

r = price([{ productId: 999, unitCode: 'CASE', qty: 1 }], cash);
eq('สินค้าที่ไม่อยู่ในชุดราคา → ปฏิเสธ (ไม่ขายผิดราคา)', [r.success, r.code], [false, 'NOT_IN_PRICE_LIST']);

r = price([{ productId: 201, unitCode: 'CASE', qty: 40 }], cash);           // 40 × 1200 = 48,000 รวม VAT → ไม่รวม VAT 44,859 < 50,000
eq('กาว 40 หีบ = 48,000 → ยังไม่ถึงโปรบิล', [r.subtotal, r.billPercent, r.total], [48000, 0, 48000]);

r = price([{ productId: 201, unitCode: 'CASE', qty: 45 }], cash);           // 54,000 รวม VAT → 50,467.29 ไม่รวม VAT ≥ 50,000
