...
> **ที่ยังไม่ปิด**: บิล `office_delivery` ยังไม่มีจุดตัดสต็อก (ดูหัวข้อ "ค้างอยู่" ท้ายไฟล์)
...
- ชุดช่องลงชื่อเดินตามลำดับงานจริงของเอกสารแต่ละชนิด (`SO_DOC_KINDS` ใน `index.html`):
  ใบส่งของ/ใบกำกับภาษี = ผู้ส่งของ·ผู้รับสินค้า · ใบเสร็จ = ผู้รับเงิน·ผู้จ่ายเงิน ·
  ใบจัดของ = ผู้จัดของ·ผู้ตรวจสอบ·ผู้รับของขึ้นรถ (ใบจัดของไม่พิมพ์ราคา)
...
Pending / not started:
- **★ บิลที่ให้สำนักงานจัดส่ง (`office_delivery`) ยังไม่ตัดสต็อกที่ไหนเลย** — ไม่แตะทั้ง `van_stock` และ
  `warehouse_stock` (คลังกลางถูกเขียนโดยงานรับของเข้าคลังเท่านั้น) · ขายจากรถ (`immediate`) ตัดตอนบันทึกบิล
  จุดเดียวถูกต้องแล้ว · ต้องให้เจ้าของระบบเลือกก่อนว่าบิลส่งจากออฟฟิศจะตัดของจากคลังไหน ที่ขั้นไหน
  (ตอนเปิดบิล / ตอน "กำลังจัดส่ง" / ตอน "ส่งของแล้ว") แล้วค่อยแก้ที่ `saleStockTaken()` ใน `34_sales_status.gs`
  ที่เดียว ทั้งจุดตัดและจุดคืนของตอนยกเลิกจะขยับตามพร้อมกัน (guide ข้อ 1.2/1.4)
- ยังไม่มี **ยอดจอง (reserved)** ตาม guide ข้อ 1.3 — ตอนนี้มีแต่ยอดคงเหลือตัวเดียว สองบิลจึงยืนยันของชิ้น
  เดียวกันได้ · จะจำเป็นเมื่อทำจุดตัดของ office_delivery เท่านั้น (ขายจากรถตัดทันทีอยู่แล้ว ไม่ต้องจอง)
