...
## ภาษีมูลค่าเพิ่มฝั่งขาย (2026-09-27)

- **ราคาขายทุกช่องของเราเป็นราคา "รวม VAT แล้ว"** — ใบรายการขายของบริษัทพิมพ์ราคารวมภาษีมาแต่ไหนแต่ไร
  ชุดราคาจึงเก็บ `cash_price_incl_vat` / `credit_price_incl_vat` ตรงๆ · ที่เก็บ `list_price_ex_vat` ไว้ด้วย
  เป็นราคาตั้งจากใบราคา ไม่ได้เอาไปคิดเงิน
- **ถอดภาษีด้วย `splitVat(gross, rate?)` ใน `18_pricing_engine.gs` ที่เดียวเท่านั้น** — ห้ามเขียน `/1.07` เองที่อื่น
  ฟังก์ชันนี้การันตีว่า `exVat + vat === gross` เป๊ะทุกยอด (มีเทสต์ไล่ 2,000 ยอดที่หารไม่ลงตัว)
  ปัดเศษคนละที่คือต้นเหตุคลาสสิกที่ทำให้ตัวเลขบนกระดาษกับในบัญชีต่างกันทีละสตางค์แล้วงบไม่ลงตัว
- **บิลขายเก็บยอดแยกภาษีไว้ตั้งแต่ตอนบันทึก** (`vat_rate`, `subtotal_ex_vat`, `vat_amount` ใน `sales_orders`)
  ไม่คำนวณย้อนหลัง — อัตราภาษีเปลี่ยนได้ และใบกำกับภาษีที่พิมพ์ไปแล้วต้องตรงกับตัวเลขในระบบตลอดไป
  · อ่านผ่าน `_orderVat(order)` ซึ่ง fallback ถอดสดให้บิลเก่าที่ยังไม่มีคอลัมน์นี้
  · ใบแจ้งหนี้ลูกหนี้ (`createArInvoice`) ใช้ตัวเลขชุดเดียวกับบิล ไม่ถอดเองซ้ำ
- **ใบกำกับภาษีต้องแสดง "มูลค่าสินค้า" กับ "ภาษีมูลค่าเพิ่ม 7%" แยกบรรทัดกัน** (กฎหมายไทยบังคับ)
  เอกสารที่ไม่พิมพ์ราคา (ใบส่งสินค้า/ใบจัดของ/ใบปะหน้าพัสดุ) ไม่มีบรรทัดภาษี
- ฝั่งซื้อมี VAT อยู่แล้ว: `po.vat_type` = included/excluded, `PO_VAT_RATE`, ลงบัญชี 1400 ภาษีซื้อ / 2200 ภาษีขาย

**ที่ยังไม่ทำ / กับดักที่ต้องรู้**
- `products.vat_type` มีคอลัมน์แต่ **ไม่มีโค้ดไหนอ่านเลย** และค่า default ที่ `addProduct` ใส่ให้คือ `'none'`
  ซึ่ง**ไม่ได้แปลว่าไม่มีภาษี** แต่แปลว่า "ยังไม่ได้ตั้ง" — ถ้าวันไหนจะทำภาษีรายสินค้า (ยกเว้น/อัตราศูนย์)
  ต้องล้างความหมายคอลัมน์นี้ก่อน ไม่งั้นสินค้าทุกตัวในระบบจะกลายเป็นไม่มีภาษีทันที
- `customers.tax_type` (vat/exempt/zero) ก็ยังไม่มีใครอ่าน
- ยังไม่มีรายงานภาษีขาย/ภาษีซื้อแยกใบ (ดูรวมได้จากงบทดลอง บัญชี 1400/2200)
